...
def discarded_method(new_name: str):
    def __depreciated__(func):
        def wrapper(*args, **kwargs):
            for tb_info in traceback.extract_stack():
                assert isinstance(tb_info, traceback.FrameSummary)
                warnings.warn('[discarded]: {} at line {} {}'.format(tb_info.filename, tb_info.lineno, tb_info.line))
            raise RuntimeError(
                'method <{}> is discarded, please use {} instead. \n\tparam detail: {} {}'.format(
                    getattr(func, '__name__', str(func)), new_name, repr(args), repr(kwargs))
            )
            # return func.__call__(*args, **kwargs)
        return wrapper
    return __depreciated__
