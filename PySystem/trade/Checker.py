...
def check_basic_info(obj: dict):
    ...
    assert product in PRODUCT_NAME_RANGE, '{}\n{}'.format(product, obj)
    date = obj.get('date')
    ...
    if '普通账户' in account_type or '两融账户' in account_type:
        if '.' not in security_code:
            if len(security_code) == 6:
                ...
                if security_code[0] in ('0', ):
                    secu_obj['security_code'] = '.'.join([security_code, 'HK'])
                else:
                    raise NotImplementedError(secu_obj)
            elif len(security_code) == 3:
                secu_obj['security_code'] = '.'.join(['0{}'.format(security_code), 'HK'])
            else:
        ...
        if '.' not in security_code:
            ...
            raise NotImplementedError(secu_obj)
    elif '期权账户' in account_type:
        if security_code.startswith('1000', ):
            secu_obj['security_code'] = '.'.join([security_code, 'SH'])
        else:
            raise NotImplementedError(secu_obj)
    else:
...
def check_margin_trade_info(trade_obj: dict):
    trade_class = check_str_element(trade_obj, 'trade_class', '交易类别')
    assert trade_class in ('融资', '融券', '担保'), '{}\n{}'.format(trade_class, trade_obj)
    trade_direction = check_str_element(trade_obj, 'trade_direction', '交易方向')
    assert trade_direction in (TRADE_DIRECTION_BUY, TRADE_DIRECTION_SELL), trade_direction
    trade_price = check_float_element(trade_obj, 'trade_price', '交易价格')
    trade_volume = check_float_element(trade_obj, 'trade_volume', '交易数量')
...
def check_future_trade_info(trade_obj: dict):
    ...
    trade_direction = check_str_element(trade_obj, 'trade_direction', '交易方向')
    assert trade_direction in (TRADE_DIRECTION_BUY, TRADE_DIRECTION_SELL), trade_direction
    trade_offset = check_str_element(trade_obj, 'trade_offset', '交易开平')
    assert trade_offset in ('开仓', '平仓')
    trade_price = check_float_element(trade_obj, 'trade_price', '交易价格')
    trade_volume = check_float_element(trade_obj, 'trade_volume', '交易数量')


def check_option_trade_info(trade_obj: dict):
    check_str_element(trade_obj, 'trade_class', '交易类别')
    # check_str_element(trade_obj, 'trade_name', '业务名称')
    trade_direction = check_str_element(trade_obj, 'trade_direction', '交易方向')
    assert trade_direction in (TRADE_DIRECTION_BUY, TRADE_DIRECTION_SELL), trade_direction
    ...
    trade_price = check_float_element(trade_obj, 'trade_price', '交易价格')
    trade_volume = check_float_element(trade_obj, 'trade_volume', '交易数量')
    trade_amount = check_float_element(trade_obj, 'trade_amount', '交易金额')
    check_str_element(trade_obj, 'trade_tag', '备兑标志')
...
def confirm_margin_trade_flow_list(flow_list: list):
    """检查普通流水要素"""
    for flow in flow_list:
        check_basic_info(flow)
        check_security_info(flow)
        if flow['security_code'].endswith('HK'):
            try:
                check_margin_trade_info(flow)
...
def confirm_option_trade_flow_list(flow_list: list):
    """检查期权流水要素"""
    for flow in flow_list:
        check_basic_info(flow)
        check_security_info(flow)
        check_option_trade_info(flow)
